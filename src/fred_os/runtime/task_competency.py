...
class TaskCompetencyOrchestrator:
    ...
    def analyze_task(
        ...
        task_class = str(s1_state["task_class"])
        uncertainty = min(1.0, max(0.0, float(s1_state["uncertainty"])))
        ...
        return {
            ...
            "requires_review": (
                ...
                or uncertainty >= float(self.config.get("task_competency.uncertainty_review_threshold"))
            ),
            "gaps": gaps,
        }
    ...
    def map_competencies(
        ...
        competencies = self.config.get(f"task_competency.route_competencies.{route.key}")
        ...
        for competency in competencies:
            providers = tuple(str(provider) for provider in providers_by_competency[str(competency)])
