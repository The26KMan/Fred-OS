...
class TaskCompetencyPlanner:
            ...
            if system_id != "S1":
                requirements.append(
                    self._system_requirement(
                        system_id,
                        "hard_gate" if system_id in getattr(route, "hard_gate_systems", ()) else "required",
                    )
                )
        for system_id in getattr(route, "optional_systems", ()):
    ...
    def _synthesize_execution(
        ...
        return {
            ...
            "optional_systems": list(getattr(route, "optional_systems", ())),
    ...
    def _constraints(self, text: str) -> tuple[str, ...]:
        ...
        return tuple(constrained[: int(self.config.get("task_planning.max_constraints", 6))])
    ...
    def _temporal_state(self, text: str, constraints: tuple[str, ...]) -> TemporalState:
        ...
        if starters and ("?" in text or re.match(rf"^({starters})\b", lowered)):
            return "ask"
        return "state"
    ...
    def _open_questions(
        ...
        questions: list[str] = []
        if len(re.findall(r"\w+", text)) < int(self.config.get("task_planning.short_input_tokens", 5)):
    ...
    def _complexity(
        ...
        if task_class == "systemic":
            score += int(self.config.get("task_planning.systemic_complexity_bonus", 2))
        if task_class in {"creative", "artistic"}:
            score += int(self.config.get("task_planning.creative_complexity_bonus", 1))
        if len(re.findall(r"\w+", lowered)) >= int(self.config.get("task_planning.long_input_tokens", 25)):
            score += 1
        if constraints:
            score += int(self.config.get("task_planning.constraint_complexity_bonus", 1))
        if governance_verdict.decision == "REVIEW":
            score += int(self.config.get("task_planning.review_complexity_bonus", 2))
        return min(score, int(self.config.get("task_planning.complexity_ceiling", 5)))

    def _objective(self, text: str) -> str:
        if not text:
            return "No user objective was supplied."
        first_sentence = re.split(r"(?<=[.!?])\s+", text, maxsplit=1)[0]
        return first_sentence[: int(self.config.get("task_planning.max_objective_chars", 320))]
