...
class RuntimeKernel:
    ...
    def boot(cls, *, root_dir: str | Path = '.', profile: str = 'development') -> 'RuntimeKernel':
        ...
        obs.emit('GLOBAL_INIT_OK', {'profile': profile})
    ...
    def process_turn(self, raw_input: str) -> dict[str, Any]:
        ...
        # S1 receives retrieval capability, but may only emit a candidate memory
        # write; it cannot persist an inferred fact by itself.
        s1 = self.registry.get('S1').process({'text': raw_input}, {'memory': self.memory, 'governance': verdict})
        route = self.router.choose(s1['task_class'], verdict.decision)
        context = {'s1': s1, 'governance': verdict}
        outputs = {'S1': s1}
        ...
        self.observability.emit('TURN_COMPLETED', {'route': route.name, 'systems': list(outputs), 's1_recall_hits': len(s1.get('memory_context', []))})
